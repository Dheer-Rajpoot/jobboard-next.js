...
export default function SkillsFilterForm({
  selectedSkills,
  setSideBarFormState,
  jobSkills,
}) {
  ...
  const handleSelectedSkill = (e, option) => {
    if (e.target.checked) {
      setSideBarFormState((prevState) => {
        const selectedSkills = [...prevState.job_skill];
        selectedSkills.push(option);
        return { ...prevState, job_skill: selectedSkills };
      });
    } else {
      setSideBarFormState((prevState) => {
        return {
          ...prevState,
          job_skill: prevState.job_skill.filter(
            (skill) => option.value != skill.value
          ),
        };
      });
    }
            ...
            {filter.options.map((option, optionIdx) => (
              <div key={option.value} className="flex items-center">
                <input
                  ...
                  onChange={(e) => handleSelectedSkill(e, option)}
