...
const CompanyDetails = ({ company, companyJobs }) => {
  const jobsGroupedByCategories = groupByArrayOfObjects(
    companyJobs,
    (c) => c.job_category[0].title
  );
  ...
  return (
    <>
      <main>
        {/* Profile background */}
        <div className="h-30 md:h-56 bg-slate-200">
          <Image
            className="object-cover h-full w-full"
            src={company.cover_image.url}
            width={2000}
            height={300}
            alt={`cover image - ${company.title} - ${company.slogan}`}
          />
        </div>
              ...
              <div className="-mt-12 mb-2">
                <div className="inline-flex -ml-1 -mt-1 sm:mb-0">
                  <Image
                    ...
                    width={80}
                    height={80}
                    alt={`logo - ${company.title} - ${company.slogan}`}
                  />
                </div>
              </div>
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-8 w-full">
          <div className="max-w-3xl mx-auto">
            <h3 className="text-xl leading-snug text-slate-800 font-bold mb-6">
              Open Positions at {company.title}
            </h3>

            {/* Job lists */}
            <div className="space-y-6">
              {jobsGroupedByCategories &&
                Object.keys(jobsGroupedByCategories).map((job_category) => {
                  const jobs = jobsGroupedByCategories[job_category];
                  return (
                    <div key={job_category}>
